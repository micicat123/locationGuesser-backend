...
describe('AppController (e2e)', () => {
  ...
  afterAll(async () => {
    await userRepository.query(
      `DELETE FROM "user" WHERE username='test@gmail.com';`,
    );
    await userRepository.query(
      `UPDATE "user" SET "firstName"='John' WHERE id=7;`,
    );

    await userRepository.query(`DELETE FROM "location" WHERE latitude=999999;`);
    await userRepository.query(
      `UPDATE "location" SET latitude=1.1523 WHERE id=1;`,
    );

    await userRepository.query(`DELETE FROM "log" WHERE "URL"='example.com';`);

    await userRepository.query(
      `DELETE FROM "guess" WHERE user_id=7 AND location_id=4;`,
    );
  });
});
